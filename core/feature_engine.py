...
class FeatureEngine:
    @staticmethod
    def calculate_metrics(df: pd.DataFrame, rsi_period=14) -> pd.DataFrame:
        ...
        plus_di = 100 * (
            df["plus_dm"].ewm(alpha=alpha, adjust=False).mean()
            / tr_smooth.replace(0, 0.001)
        )
        minus_di = 100 * (
            df["minus_dm"].ewm(alpha=alpha, adjust=False).mean()
            / tr_smooth.replace(0, 0.001)
        )
        ...
        roll_up, roll_down = (
            up.rolling(rsi_period).mean(),
            down.rolling(rsi_period).mean(),
        )
        RS = roll_up / roll_down.replace(0, np.nan)
        RSI = 100 - (100 / (1 + RS))
        min_rsi, max_rsi = RSI.rolling(rsi_period).min(), RSI.rolling(rsi_period).max()
        df["rsi"] = RSI
        ...
        df["setup"] = (
            (df["stoch_k"] < 0.2) & (df["ema_fast"] > df["ema_slow"]) & (df["adx"] > 20)
        )
    ...
    @staticmethod
    def get_snapshot(
        df: pd.DataFrame, idx: int, regime: str, slope: float
    ) -> FeatureSnapshot:
        row = df.iloc[idx]
